...
bool left_eye_closed = false;
bool right_eye_closed = false;
int64_t left_eye_close_start = 0;
int64_t right_eye_close_start = 0;
...
void sendUDPMessage(int64_t now, float x, float y)
{
	...
	if (x >= 0 && y >= 0) {
		size_t size = sprintf_s(data, sizeof(data), "%lld,%f,%f", 
			now, x, y);
		
		// Show normalized coordinates and screen position
		printf("Eye pos: (%.3f, %.3f) Screen pos: (%d, %d)   \r",
			x, y, 
...
static void gaze_point_callback(tobii_gaze_point_t const* gaze_point, void* user_data)
{
	...
	if (gaze_point->validity == TOBII_VALIDITY_VALID) {
		double x = double(gaze_point->position_xy[0]);
		double y = double(gaze_point->position_xy[1]);

		// Send coordinates for cursor movement
...
void eye_data_callback(tobii_gaze_data_t const* gaze_data, void* user_data) 
{
    int64_t elapsed = gaze_data->timestamp_system_us - base_time;
    
    // Check left eye
    if (gaze_data->left.pupil_validity == TOBII_VALIDITY_VALID) {
        if (gaze_data->left.pupil_diameter_mm < 2.0) { // Threshold for closed eye
            if (!left_eye_closed) {
                left_eye_close_start = elapsed;
    ...
    // Check right eye
    if (gaze_data->right.pupil_validity == TOBII_VALIDITY_VALID) {
        if (gaze_data->right.pupil_diameter_mm < 2.0) { // Threshold for closed eye
            if (!right_eye_closed) {
                right_eye_close_start = elapsed;
    ...
    // Send eye closure data via UDP
    char data[100];
    memset(data, 0, sizeof(data));
    size_t size = sprintf_s(data, sizeof(data), "eye:%lld,%d,%d,%lld,%lld",
        elapsed,
        left_eye_closed ? 1 : 0,
        right_eye_closed ? 1 : 0,
        left_eye_close_start,
        right_eye_close_start);
...
//   struct timeval tv;
  ...
  InetPton(addr.sin_family, _T(ADDRESS), &addr.sin_addr.S_un.S_addr);
	...
	error = tobii_gaze_data_subscribe(device, eye_data_callback, nullptr);
	if (error != TOBII_ERROR_NO_ERROR) {
		printf("Failed to subscribe to eye data stream\n");
		// Handle error...
	}
